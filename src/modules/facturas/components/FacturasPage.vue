<template>
  <q-page class="q-pa-md">
      ...
      <q-list bordered separator v-if="facturasFiltradas.length">
        <q-item v-for="factura in facturasFiltradas" :key="factura.ID_FACTURA_ENC">
          <q-item-section>
            ...
            <q-item-label>Total: Q{{ factura.TOTAL_GENERAL.toFixed(2) }}</q-item-label>
            <q-item-label>Fecha: {{ factura.FECHA_DE_FACTURA }}</q-item-label>
          </q-item-section>
    ...
    <!-- Detalle de la Factura en un Dialog -->
...
const facturasFiltradas = computed(() => {
  return (facturasData.value ?? []).filter(f =>
